...
function DonatorPageComponent() {
  ...
  const anuleazaProgramarea = (e, id) => {
    e.preventDefault();
     ProgramariService.anuleaza(id).then(() => {
      ProgramariService.getAllProgramariWithPagination(donator.id , paginaCurenta , numarAfisari).then((response) => {
  ...
  return (
    <div>
      ...
      {!loading && (
        <>
          <LocatiiComponent donator={donator} programari={programari} setProgramari={setProgramari}/>
          <br></br>
          <table class="table">
            <thead>
              <tr>
          ...
          <button onClick={decrementPage} className="btn btn-danger"> Back</button>
          <button onClick={incrementPage} className="btn btn-primary" >  Next </button>
          </div>
        </>
      )}
    </div>
